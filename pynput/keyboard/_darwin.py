...
class Listener(ListenerMixin, _base.Listener):
    ...
    def _handle(self, _proxy, event_type, event, _refcon):
        ...
        # FIXME: I am not yet entirely sure about this issue, but pynput crashes
        # at times when capslock is pressed and will seemingly misidentify the 
        # key as the volume up key. This appears to fix that issue. Also, this
        # only appears to happen when another even loop is running, such as Qt.
        if hasattr(key, 'value') and key.value.vk == 0:
            return
